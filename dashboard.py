...
grouped_customers = data.groupby(['device_id', 'place', 'day_of_week']).agg({
    'customer_weight':
    'sum'
}).reset_index()
...
# Grouping data
grouped_visits = visits.groupby(['place', 'distance_from_home_miles']).agg({
    'visit_weight':
    'sum'
}).reset_index()
...
# Grouping data
grouped_visits = visits.groupby(['place', 'distance_from_work_miles']).agg({
    'visit_weight':
    'sum'
}).reset_index()
grouped_customers = data.groupby(
